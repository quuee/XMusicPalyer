...
class LyricUtil{
    companion object{
         ...
         * @param lrcText
         */
        fun parseLyric(lrcText: String): List<LyricLine>? {
            if (lrcText.isEmpty()) {
                return null
            }
            val entityList: MutableList<LyricLine> = ArrayList<LyricLine>()
            // 以换行符为分割点
            val array = lrcText.split("\\n".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            for (line in array) {
                // 循环遍历按行解析
                val list: List<LyricLine>? = parseLine(line)
                list?.let {
        ...
        private fun parseLine(line: String): List<LyricLine>? {
            var newLine = line
            val entryList: MutableList<LyricLine> = ArrayList<LyricLine>()
            if (newLine.isEmpty()) {
                return null
            }
            // 去除空格
            ...
            try {
                if (!lineMatcher.matches()) {
                    if (!PATTERN_TIME.matcher(newLine).matches()){
                        entryList.add(LyricLine(0L,newLine))
                    }else{
                ...
                if (timeMatcher != null) {
                    ...
                    while (timeMatcher.find()) {
                        ...
                        entryList.add(LyricLine(content = text ?: "", time = time))
                    }
                }
            }catch (e:Exception){
                println(e.message)
        ...
        fun findCurrentLyricIndex(lyrics: List<LyricLine>, currentPosition: Long): Int {
            if (lyrics.isEmpty()) return -1
            var index = lyrics.binarySearchBy(currentPosition) { it.time }
            if (index < 0) index = -index - 2
            return index.coerceIn(0 until lyrics.size)
        }
    }
